...
import os
import torch
from var_dict import PRESSURE_VARS, SURFACE_VARS, atmos_level_herbie
...
def get_args(argv=None):  
    ...
    parser.add_argument('-figure_dir', '--figure_dir', default="figure_diffLT_acc", type=str, help='figure_dir')  
...
data_dir = args.file_dir # "Nwp_acc_loss_file_two_dataset"
figure_dir = args.figure_dir # figure_diffLT_acc
...
for var_name in SURFACE_VARS:
    ...
    path_save_nwp = f"./{figure_dir}/figure_nwp_{var_mapping_key}.png"
    print(path_save_nwp)
    ...
    nwp_xlsx[var_mapping_key] = np.array(mean_leadtime_plot_nwp)
    persistent_xlsx[var_mapping_key] = np.array(mean_leadtime_plot_persistent)
...
import pandas as pd
pd.DataFrame(nwp_xlsx).to_excel(f"./{data_dir}/202007to12/nwp_loss.xlsx")
pd.DataFrame(persistent_xlsx).to_excel(f"./{data_dir}/202007to12/persistent_loss.xlsx")
