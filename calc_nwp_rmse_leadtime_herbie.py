...
for day in day_list:
    for hour in hour_list:
        ...
        try:  
            H_input = Herbie(f"{file_name_input[0]} {file_name_input[1]}:00", model="hrrr", product="prs", fxx=0)
            H_true = Herbie(f"{file_name_true[0]} {file_name_true[1]}:00", model="hrrr", product="prs", fxx=0)
            H_pre = Herbie(f"{file_name_pre[0]} {file_name_pre[1]}:00", model="hrrr", product="prs", fxx=args.leadtime)
            ...
            for var in PRESSURE_VARS+SURFACE_VARS:
                if var in PRESSURE_VARS:
                    for level_ in atmos_level_herbie:
                        ...
                        # 记录图像集合  
                        images = [wandb.Image(input_array), wandb.Image(true_array), wandb.Image(pre_array)]  
                        wandb_log_dict[f"{var}_{level_}_input_true_pre"] = images
                        print(var, level_, input_array.shape, true_array.shape, pre_array.shape)
                else:
                    ...
                    print(var, input_array.shape, true_array.shape, pre_array.shape)                    
            t2 = time.time()
