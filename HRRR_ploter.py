        ...
        for var in PRESSURE_VARS+SURFACE_VARS:
            if var in PRESSURE_VARS:
                ...
                for level_index in range(atmos_var_pre_value.shape[0]):
                    nwp_loss = np.sqrt(((atmos_var_true_value[level_index] - atmos_var_pre_value[level_index])**2).mean())
                    persistent_loss = np.sqrt(((atmos_var_true_value[level_index] - atmos_var_input_value[level_index])**2).mean())
                    # 绘制1排3列的图像  
                    ploter(atmos_var_input_value[level_index], 
                           atmos_var_true_value[level_index], 
                           atmos_var_pre_value[level_index],
                           f"{var}_{level_index} nwp_loss:{nwp_loss:.4f} persistent_loss:{persistent_loss:.4f}",
                           f"{day}_{hour}_{var}_{level_index}.png")
            else:
                if var == "UGRD_P0_L103_GLC0" or var == "VGRD_P0_L103_GLC0":
                    ...
                    atmos_var_input_value = ds_input[var].to_numpy()
                print(var, atmos_var_true_value.shape, atmos_var_pre_value.shape, atmos_var_input_value.shape)
                nwp_loss = np.sqrt(((atmos_var_true_value - atmos_var_pre_value)**2).mean())
                persistent_loss = np.sqrt(((atmos_var_true_value - atmos_var_input_value)**2).mean())
                # 绘制1排3列的图像  
                ploter(atmos_var_input_value, atmos_var_true_value, atmos_var_pre_value,
                       f"{var} nwp_loss:{nwp_loss:.2f} persistent_loss:{persistent_loss:.2f}",
                       f"{day}_{hour}_{var}.png")
        t2 = time.time()
